...
-- Auto-generate ID for elements without explicit IDs.
local _autoIdCounter = 0
local function _generateId()
  _autoIdCounter = _autoIdCounter + 1
  return "el_" .. _autoIdCounter
end
...
function Element.new(props, _parent)
  ...
  if props.children then
    if type(props.children) ~= "table" then
      ...
      -- Iterate by numeric key so sparse arrays (e.g. `{nil, {...}, nil}`) are
      -- handled without # returning 0 and skipping the surviving entries.
      local maxIndex = 0
      for k in pairs(props.children) do
        if type(k) == "number" and k > maxIndex then
          maxIndex = k
        end
      end
      for i = 1, maxIndex do
        local entry = props.children[i]
        if entry == nil then
          Element._ErrorHandler:warn("Element", "ELEM_011", {
            element = self.id or "unnamed",
            issue = "nil entry in children array, skipping",
          })
        elseif type(entry) == "table" and getmetatable(entry) == Element then
          -- Pre-constructed Element instance: reparent it to this element.
...
function Element:draw(backdropCanvas)
  ...
  if self.display == "none" or self.opacity <= 0 or self.visibility == "hidden" then
    return
  end
...
function Element:update(dt)
  ...
  if self.onEvent or self.themeComponent or self.editable or self._selectState or self.selectOption then
    ...
    -- This prevents occluded elements from receiving interactions or visual feedback.
    local isActiveElement = (
      Element._Context._activeEventElement == nil or Element._Context._activeEventElement == self
    )
    ...
    -- Pick up direct mutations of theme/disabled/active each frame.
    if self._themeManager.themeComponent ~= self.themeComponent then
      self._themeManager.themeComponent = self.themeComponent
      if self._renderer then
        self._renderer.themeComponent = self.themeComponent
      end
    end
    if self._themeManager.disabled ~= self.disabled then
      self._themeManager.disabled = self.disabled
    end
    if self._themeManager.active ~= self.active then
      self._themeManager.active = self.active
    end
